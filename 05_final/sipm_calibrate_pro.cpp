...
void sipm_calibrate_pro()
{
    ...
    // pay attention to constexpr memory
    ...
    for (int vbias : vbiasList) {
        ...
        // ── Collect run files ─────────────────────────────────────────────────
        // patA: "data.vbias_<V>_run_"  (vecchio schema)
        // patB: "_vbias_<V>.root"      (cattura data_x_..._y_..._vbias_V[_run_N].root)
        // patB e' sufficiente per il nuovo schema: cattura TUTTE le posizioni.
        // RIMOSSO patC hardcoded "x_90_y_0": filtrava solo il centro fascio e
        // saltava silenziosamente tutti gli altri file validi.
        const std::string patA = makeRunPattern(vbias);
        const std::string patB = "_vbias_" + std::to_string(vbias);
        ...
        // Aggiunge file finche' non si raggiunge MAX_EVENTS.
        // TChain::Add(fp, -1) legge il numero di entries dall'header del file
        // SENZA aprire/scansionare tutto — molto piu' veloce di TFile::Open.
        Long64_t accumulated = 0;
        int nFilesAdded = 0;
        for (const auto& fp : fileList) {
            if (accumulated >= MAX_EVENTS) break;
            chain->Add(fp.c_str());  // -1 default: legge entries dall'header
            ++nFilesAdded;
            accumulated = chain->GetEntries();  // aggiornamento incrementale
        }
        const Long64_t totalEvents = chain->GetEntries();
        std::cout << "  Using " << nFilesAdded << "/" << fileList.size()
                  << " file(s), " << totalEvents << " events"
                  << (totalEvents >= MAX_EVENTS
                      ? " (>= cap " + std::to_string(MAX_EVENTS) + ")" : "")
                  << "\n";

        // ── Calibration ───────────────────────────────────────────────────────
        // Delega interamente a calibrateSpectrum() da Calibration.h.
        // Tutte le costanti vengono da Config.h.
        std::cout << "  Calibrating — please wait...\n";
        ...
        CalibResult cal = calibrateSpectrum(
            chain, cutoff_MHz, fs_MHz, calTag, ctx, &scanData, highLight);
