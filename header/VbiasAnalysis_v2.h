        ...
        int vbias, double frac_pe, bool use_filter)
{
    ...
    const int min_fall_samp = 3;  // l'isteresi in discesa fa il lavoro pesante
    ...
    for (auto& [runNum, path] : runFiles) {
        ...
        for (Long64_t i = 0; i < nFile; ++i) {
            ...
            auto [t_rise, t_fall] = computeTOT(v_t, af, let_thr,
                ...
                100, edge_thr_frac, 0.5, 10, 0.3, min_fall_samp, 50, false);
        ...
        const std::string& ltag, int vbias, OutCtx& ctx)
{
    ...
    TH2D*h2D=new TH2D(Form("h2D_%s",ltag.c_str()),Form("TOT vs #Deltat LET=%.2f p.e. %s;TOT (ns);#Deltat (ns)",frac,ltag.c_str()),300,0,150,1000,-50,200);h2D->SetDirectory(nullptr);
    ...
    {TCanvas*c=new TCanvas(Form("c2D_%s",ltag.c_str()),"",900,700);c->SetRightMargin(0.15);c->SetLeftMargin(PAD_LEFT);c->SetBottomMargin(PAD_BOTTOM);c->SetTopMargin(PAD_TOP);c->SetLogz();c->SetGrid();h2D->Draw("COLZ");c->Update();c->Modified();ctx.savePNG(c,Form("tot_map_%s.png",ltag.c_str()));}
    ...
    {TCanvas*c=new TCanvas(Form("cPr_%s",ltag.c_str()),"",900,550);c->SetGrid();c->SetLeftMargin(PAD_LEFT);c->SetRightMargin(PAD_RIGHT);c->SetBottomMargin(PAD_BOTTOM);c->SetTopMargin(PAD_TOP);
     ...
     hPr->SetLineColor(kAzure+1);hPr->SetLineWidth(2);hPr->Draw("HIST");TF1*fG=new TF1(Form("fG_%s",ltag.c_str()),"gaus",std::max(fit_lo,ctr-3*sg),std::min(fit_hi,ctr+3*sg));fG->SetParameters(hPr->GetMaximum(),ctr,sg);hPr->Fit(fG,"RQ");fG->SetLineColor(kRed+1);fG->Draw("same");
     TPaveText*ptG=new TPaveText(0.55,0.72,0.93,0.88,"NDC");ptG->SetBorderSize(1);ptG->SetFillColor(0);ptG->SetTextFont(42);ptG->SetTextSize(0.036);ptG->AddText(Form("#sigma=%.3f#pm%.3f ns",std::abs(fG->GetParameter(2)),fG->GetParError(2)));ptG->AddText(Form("#mu=%.3f ns",fG->GetParameter(1)));ptG->Draw();
    ...
    if(tw_method==TWMethod::EMPIRICAL){double tm=0;TF1*fTW=computeTimeWalkFromCache(cachePath,fit_lo,fit_hi,ltag,tm);
     if(fTW){double ref=fTW->Eval(tm);TH2D*h2Dc=new TH2D(Form("h2Dc_%s",ltag.c_str()),"",300,0,150,1000,-50,200);h2Dc->SetDirectory(nullptr);
     ...
     if(!mp.empty()){TH2D*h2Dc=new TH2D(Form("h2Dc_%s",ltag.c_str()),"",300,0,150,1000,-50,200);h2Dc->SetDirectory(nullptr);
        ...
        OutCtx& ctx, const CalibResult& cal_in)
{
    ...
    double fs_MHz=0;
    {TFile*f0=TFile::Open(foundRuns.begin()->second.c_str(),"READ");if(!f0||f0->IsZombie())return sigmaOut;TTree*tr=(TTree*)f0->Get("ch1");if(tr){const int N0=1024;Double_t tb[N0];tr->SetBranchAddress("time",tb);tr->GetEntry(0);fs_MHz=1000./(tb[1]-tb[0]);}f0->Close();delete f0;}
    if(fs_MHz<=0)return sigmaOut;
