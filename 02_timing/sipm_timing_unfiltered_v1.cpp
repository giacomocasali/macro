...
void sipm_timing_unfiltered_v1() {
    gStyle->SetOptFit(0);
    gStyle->SetOptStat(0);
    ...
    // Label with mean ± error + sigma for each peak
    TLegend *legSpec = new TLegend(0.48, 0.28, 0.95, 0.92);
    legSpec->SetBorderSize(1); legSpec->SetFillColor(0);
    legSpec->SetTextFont(42); legSpec->SetTextSize(0.018);
    legSpec->SetHeader("Peak    #mu #pm #delta#mu (mV)    #sigma (mV)", "C");
    for(int i=0; i<nPeaks; i++) {
        TF1 *gi_leg = new TF1(Form("gi_leg_%d", i), "gaus", -5, 60);
        gi_leg->SetParameters(fitFunc->GetParameter(1+3*i), fitFunc->GetParameter(2+3*i), fitFunc->GetParameter(3+3*i));
        gi_leg->SetLineColor(kAzure-4+i); gi_leg->SetLineStyle(2);
        legSpec->AddEntry(gi_leg, Form("  %d    %.3f #pm %.3f    %.3f", i,
            fitFunc->GetParameter(2+3*i), fitFunc->GetParError(2+3*i),
            fitFunc->GetParameter(3+3*i)), "l");
    }
    legSpec->Draw();
    ...
    // FIX: chi2 in legend, more decimal places for m
    ...
    for(int n=1; n<nPeaks; n++) {
        ...
        // FIX: rebin for better visual resolution
    ...
    std::cout << "\nAnalysis Complete (unfiltered). LET: " << let << " mV" << std::endl;
    ...
    cAmp->SaveAs("uf_plot_01_spectrum.png");
    cLin->SaveAs("uf_plot_02_linearity.png");
    cTime->SaveAs("uf_plot_03_timing_LET.png");
    c2D->SaveAs("uf_plot_04_2Dmap_LET.png");
    cProj->SaveAs("uf_plot_05_amplitude_projection.png");
    cOverlay->SaveAs("uf_plot_06_overlay.png");
    cSlices->SaveAs("uf_plot_07_timing_slices.png");
    cMeanTrend->SaveAs("uf_plot_08_mean_trend.png");
    cResTrend->SaveAs("uf_plot_09_resolution_trend.png");
    cTimeCFD->SaveAs("uf_plot_10_timing_CFD.png");
    c2DCFD->SaveAs("uf_plot_11_2Dmap_CFD.png");
    cSlicesCFD->SaveAs("uf_plot_12_slices_CFD.png");
    cMeanCFD->SaveAs("uf_plot_13_mean_trend_CFD.png");
    cResCFD->SaveAs("uf_plot_14_resolution_CFD.png");
    std::cout << "All canvases saved (prefix: uf_)." << std::endl;
