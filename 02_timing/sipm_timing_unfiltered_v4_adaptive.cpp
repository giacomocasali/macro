...
// UNFILTERED VERSION v7: adaptive zoom, 4 decimal m, Constraints REMOVED
...
static std::tuple<double,double,double,double,TF1*> q_gauss_projection_zoom(TH1D* hProj, double max_window = 2.0) {
    ...
    double sigmaEst = std::max(fwhm / 2.35, 2.0 * binW); 
...
void sipm_timing_unfiltered_v4_adaptive() {
    gStyle->SetOptFit(0);
    gStyle->SetOptStat(0);
    ...
    for(Long64_t i=0; i<treeCh1->GetEntries(); i++) {
        // Esc key or general interrupt stop trigger
        if(i % 500 == 0) {
            if (gSystem->ProcessEvents() || gROOT->IsInterrupted()) break;
        }
    ...
    for(int i=0; i<nPeaks; i++) {
        ...
        // CONSTRAINTS REMOVED: Allow fit to locate the mean naturally
        // fitFunc->SetParLimits(2+3*i, x_est - 1.0, x_est + 1.0);
        
        fitFunc->SetParameter(3+3*i, 0.6);
        // CONSTRAINTS REMOVED: Allow fit to adjust sigma naturally
