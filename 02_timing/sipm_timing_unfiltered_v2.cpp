...
// UNFILTERED VERSION: full waveform, no amplitude/time cuts
...
// =========================================================
// FROM gauss_stuff.h — q_gauss_projection with zoom window
// =========================================================
...
static std::tuple<double,double,double,double,TF1*> q_gauss_projection_zoom(TH1D* hProj, double window = 2.0) {
    if (!hProj || hProj->GetEntries() == 0) return {0,0,0,0,nullptr};
    int maxBin     = hProj->GetMaximumBin();
    double center  = hProj->GetBinCenter(maxBin);
    double fitMin  = center - window;
    double fitMax  = center + window;
    double A_guess     = hProj->GetMaximum();
    double mean_guess  = center;
    double sigma_guess = 0.3;
    ...
    fFit->SetNpx(1000);
    hProj->GetXaxis()->SetRangeUser(center - 3*window, center + 3*window);
...
// =========================================================
// MAIN ANALYSIS FUNCTION — UNFILTERED
// =========================================================
...
void sipm_timing_unfiltered_v2() {
    gStyle->SetOptFit(0);
    gStyle->SetOptStat(0);
    ...
    // Label with mean ± error + sigma — no color column, text-only rows
    TLegend *legSpec = new TLegend(0.42, 0.28, 0.95, 0.92);
    legSpec->SetBorderSize(1); legSpec->SetFillColor(0);
    legSpec->SetTextFont(82); // monospace throughout
    legSpec->SetTextSize(0.022);
    legSpec->SetMargin(0.0); // no symbol column at all
    legSpec->SetHeader("#font[82]{Peak     #mu #pm #delta#mu (mV)    #sigma (mV)}", "C");
    for(int i=0; i<nPeaks; i++) {
        // "" as option = no symbol drawn, pure text row
        legSpec->AddEntry((TObject*)0,
            Form("#font[82]{ %d      %6.3f  #pm  %5.3f      %5.3f}", i,
                fitFunc->GetParameter(2+3*i), fitFunc->GetParError(2+3*i),
                fitFunc->GetParameter(3+3*i)), "");
    }
    legSpec->Draw();

    // --- CANVAS 01-LIN: SPECTRUM LINEAR SCALE (new plot 1) ---
    // Same multigaussian fit as cAmp but without SetLogy().
    // Identical legend so mu, delta_mu, sigma values are the same.
    TCanvas *cAmpLin = new TCanvas("cAmpLin", "SiPM Amplitude Spectrum Linear Scale (Unfiltered)", 900, 900);
    cAmpLin->SetLeftMargin(0.15);
    // NO SetLogy — linear scale intentional
    TH1D *hTempLin = (TH1D*)hTemp->Clone("hTempLin");
    hTempLin->SetLineColor(kBlack);
    hTempLin->GetYaxis()->SetTitleOffset(1.6);
    hTempLin->Draw("HIST");
    TF1 *fitFuncLin = (TF1*)fitFunc->Clone("fitFuncLin");
    fitFuncLin->SetLineColor(kRed); fitFuncLin->SetLineWidth(2);
    fitFuncLin->Draw("SAME");
    for(int i=0; i<nPeaks; i++) {
        TF1 *giLin = new TF1(Form("giLin_%d", i), "gaus", -5, 60);
        giLin->SetParameters(fitFunc->GetParameter(1+3*i), fitFunc->GetParameter(2+3*i), fitFunc->GetParameter(3+3*i));
        giLin->SetLineColor(kAzure-4+i); giLin->SetLineStyle(2); giLin->Draw("SAME");
    }
    // Identical legend layout to cAmp — no color column, text-only rows
    TLegend *legSpecLin = new TLegend(0.42, 0.50, 0.95, 0.92);
    legSpecLin->SetBorderSize(1); legSpecLin->SetFillColor(0);
    legSpecLin->SetTextFont(82); // monospace throughout
    legSpecLin->SetTextSize(0.022);
    legSpecLin->SetMargin(0.0); // no symbol column at all
    legSpecLin->SetHeader("#font[82]{Peak     #mu #pm #delta#mu (mV)    #sigma (mV)}", "C");
    for(int i=0; i<nPeaks; i++) {
        legSpecLin->AddEntry((TObject*)0,
            Form("#font[82]{ %d      %6.3f  #pm  %5.3f      %5.3f}", i,
                fitFunc->GetParameter(2+3*i), fitFunc->GetParError(2+3*i),
                fitFunc->GetParameter(3+3*i)), "");
    }
    ...
    // gr: data points with yErrMean (Minuit error on mu) as Y-error bars.
    // Fit also uses yErrMean so that m, q, and let are correct.
    // The sigma band (grSigma) shows intrinsic peak width separately.
    ...
    linFit->SetLineColor(kRed+1); gr->Fit(linFit, "RQ");
    linFit->Draw("SAME"); // explicitly draw the fit line
    gr->Draw("PE SAME"); // PE = markers + error bars
    TLegend *legLin = new TLegend(0.15, 0.60, 0.55, 0.88);
    legLin->SetBorderSize(1); legLin->SetTextSize(0.028);
    legLin->AddEntry(gr,      "Data #mu #pm #delta#mu", "pe");
    legLin->AddEntry(grSigma, "#sigma band (peak width)", "f");
    legLin->AddEntry(linFit,  "Linear fit", "l");
    ...
    // =========================================================
    // PASS 2 — Timing: full waveform, range -50 to 200 ns
    // =========================================================
    // FIX: range extended to -50 to 200 ns, higher binning 500x500
    TH1D *hTime = new TH1D("hTime", Form("LET Timing Spectrum @ %.2f mV (Unfiltered);#Deltat (ns);counts", let), 500, -50, 200);
    TH2D *h2D   = new TH2D("h2D",   Form("Amplitude vs LET Arrival Time @ %.2f mV;amplitude (mV);t_{LET} - t_{laser} (ns)", let), 400, -5, 60, 1000, -50, 200);
    ...
    // --- CANVAS 6: OVERLAY — FIX: no fill on projection, only line ---
    ...
    hTempC6->SetLineWidth(2);
    hTempC6->SetFillStyle(0); // no fill
    hTempC6->Draw("HIST");
    hProjC6->SetLineColor(kSpring-1); // green pea color
    hProjC6->SetLineWidth(2);
    hProjC6->SetFillStyle(0); // FIX: no fill, only line
    ...
    legOver->AddEntry(hTempC6, "Total spectrum (all events, Pass 1)", "l");
    legOver->AddEntry(hProjC6, "Timing-gated projection (laser trigger + LET crossing required)", "l");
    // NOTE: the two samples are NOT identical — hTemp includes events without
    // a valid laser trigger, while hProjX requires both a laser trigger and a
    // LET threshold crossing. The difference reflects both the LET amplitude
    // cut and events lost due to missing laser trigger, not only the LET cut.
    legOver->Draw();

    // --- CANVAS 7: TIMING SLICES — higher binning, logy, zoom fit ---
    ...
    for(int n=1; n<nPeaks; n++) {
        ...
        gPad->SetLogy();
        // No SetGrid() — remove dashed internal grid lines per request
        ...
        // No rebin: use native bin width (0.25 ns) for maximum sensitivity
        TH1D *hSlice = h2D->ProjectionY(Form("hSlice_%d", n), binLow, binHigh);
        // hSlice->Rebin(1); // identity — kept for clarity, no rebinning applied
        hSlice->SetTitle(Form("%d p.e. — LET Timing;#Deltat (ns);counts", n));
        hSlice->GetYaxis()->SetTitleOffset(2.0);
        hSlice->GetYaxis()->SetTitleSize(0.07);
        hSlice->GetXaxis()->SetTitleSize(0.07);
    ...
    // FIX: force positive parameters — sqrt(s^2/n + c^2) is symmetric in sign
    // so without limits ROOT may converge to negative values which are unphysical.
    fitSigma->SetParLimits(0, 0.0, 10.0); // sigma_stoc >= 0
    fitSigma->SetParLimits(1, 0.0, 10.0); // sigma_const >= 0
    fitSigma->SetLineColor(kBlack); fitSigma->SetLineStyle(2);
    grRes->Fit(fitSigma, "RQB"); grRes->Draw("AP"); // "B" enforces parameter limits
    ...
    TH2D *h2DCFD   = new TH2D("h2DCFD",   "Amplitude vs CFD Arrival Time (Unfiltered);amplitude (mV);t_{CFD} - t_{laser} (ns)", 400, -5, 60, 1000, -50, 200);
    ...
    h2DCFD->SetContour(100); h2DCFD->GetYaxis()->SetTitleOffset(1.6);
    h2DCFD->Draw("COLZ");
    ...
    for(int n=1; n<nPeaks; n++) {
        ...
        // No rebin: use native bin width (0.25 ns) for maximum sensitivity
        // hS->Rebin(1); // identity — kept for clarity, no rebinning applied
        ...
        hS->SetLineColor(kRed-7); hS->Draw("HIST");
    ...
    fSC->SetParameters(0.2, 0.05);
    // FIX: same as fitSigma — force positive parameters to avoid unphysical negative sigma.
    fSC->SetParLimits(0, 0.0, 10.0); // sigma_stoc >= 0
    fSC->SetParLimits(1, 0.0, 10.0); // sigma_const >= 0
    fSC->SetLineColor(kRed); grResC->Fit(fSC, "RQB"); grResC->Draw("AP"); // "B" enforces limits
    ...
    // --- SAVE ALL 15 CANVASES AS PNG into plots/ subfolder ---
    gSystem->Exec("mkdir -p plots");
    cAmpLin->SaveAs("plots/uf_plot_01_spectrum_linear.png");
    cAmp->SaveAs("plots/uf_plot_02_spectrum_log.png");
    cLin->SaveAs("plots/uf_plot_03_linearity.png");
    cTime->SaveAs("plots/uf_plot_04_timing_LET.png");
    c2D->SaveAs("plots/uf_plot_05_2Dmap_LET.png");
    cProj->SaveAs("plots/uf_plot_06_amplitude_projection.png");
    cOverlay->SaveAs("plots/uf_plot_07_overlay.png");
    cSlices->SaveAs("plots/uf_plot_08_timing_slices_LET.png");
    cMeanTrend->SaveAs("plots/uf_plot_09_mean_trend_LET.png");
    cResTrend->SaveAs("plots/uf_plot_10_resolution_LET.png");
    cTimeCFD->SaveAs("plots/uf_plot_11_timing_CFD.png");
    c2DCFD->SaveAs("plots/uf_plot_12_2Dmap_CFD.png");
    cSlicesCFD->SaveAs("plots/uf_plot_13_slices_CFD.png");
    cMeanCFD->SaveAs("plots/uf_plot_14_mean_trend_CFD.png");
    cResCFD->SaveAs("plots/uf_plot_15_resolution_CFD.png");
